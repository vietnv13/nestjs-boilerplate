...
const headerVariants = cva('flex items-center justify-between px-4 py-3 w-full', {
  variants: {
    variant: {
      static: '',
      sticky:
...
function Header({ className, variant, ...props }: HeaderProps) {
  return (
    <header data-slot="header" className={cn(headerVariants({ variant, className }))} {...props} />
  )
}
...
function HeaderStart({ className, ...props }: HeaderStartProps) {
  return (
    <div data-slot="header-start" className={cn('flex items-center gap-2', className)} {...props} />
  )
}
...
export { Header, HeaderCenter, HeaderEnd, HeaderStart, headerVariants }
