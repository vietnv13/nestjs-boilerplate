...
class AdminSseClient {
  ...
  /**
   * Force a reconnect cycle (useful when server emits a domain "error" event).
   */
  forceReconnect() {
  ...
  private scheduleReconnect() {
    if (this.refCount === 0) return
    if (this.eventSource) return // EventSource will auto-retry; we only recreate if it looks stuck.
    if (!this.currentUrl) return
...
// Module-level singleton so the SSE connection is shared across the whole app.
export const adminSseClient = new AdminSseClient()
