...
| ------------------------------------------------ | ---------------------------------------------------- |
| [Architecture Overview](./architecture.md)       | Layer structure, modules, request pipeline, packages |
...
- **Reusable packages** — HTTP interceptors/middleware, cache, queue, scheduler, storage are all workspace packages under `packages/`
- **`@workspace/nestjs-http`** — drop-in HTTP infrastructure: ETag, versioning, correlation IDs, pagination links, timeout, and more
