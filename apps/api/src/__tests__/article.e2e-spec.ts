...
describe("article E2E Tests", () => {
  ...
  describe("/articles (POST) - Create article", () => {
    it("should create draft article successfully", async () => {
      ...
      const response = await createRequest(app).post("/articles").send(createDto).expect(201);
    ...
    it("should return 400 when title is too short", async () => {
      ...
      const response = await createRequest(app).post("/articles").send(createDto).expect(400);
  ...
  describe("/articles/:id (GET) - Get article", () => {
    it("should get created article successfully", async () => {
      const response = await createRequest(app).get(`/articles/${createdArticleId}`).expect(200);
    ...
    it("should return 404 when article not found", async () => {
      ...
      const response = await createRequest(app).get(`/articles/${nonExistentId}`).expect(404);
  ...
  describe("/articles (GET) - Get article list", () => {
    it("should return article list", async () => {
      const response = await createRequest(app).get("/articles").expect(200);
      ...
      const ourArticle = articles.find((article) => article.id === createdArticleId);
